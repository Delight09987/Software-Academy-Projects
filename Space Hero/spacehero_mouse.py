...
class Enemy ():
    ...
    def move(self): # colon means to give or present a command/task
        self.x = self.x + self.dx
        self.y = self.y + self.dy
        if self.type == "scary spaceship":
            self.angle += 10 # it will make it spin faster
            pygame.transform.rotate(self.surface, 1)
        if self.x < -30:
    ...
    def render(self):
        if self.type == "scary spaceship":
            self.rota_image = pygame.transform.rotate(self.surface, self.angle)
            screen.blit(self.rota_image, (int(self.x), int(self.y)))
            pygame.draw.line(screen, RED, (int(self.x), int(self.y)), (int(self.x + (60 *(self.health/self.max_health))), int(self.y)), 2)
        else:
            screen.blit(self.surface, (int(self.x), int(self.y)))
            pygame.draw.line(screen, RED,(int(self.x), int(self.y)), (int(self.x + (40 * (self.health/self.max_health))), int(self.y)), 2)
...
class Highscores():
    ...
    def load(self):
        try: # to check or inspect something typically debugging
            with open(self.filename, "r") as f:# r  for read, f for file
                for line in f:
                    name, score, kills = line.strip() .split(',')
                        # strip removes white spaces
                    self.scores.append((name, int(score), int(kills)))
    ...
    def add_score(self,name,score,kills): # when game is complete, run this to add a score
        self.scores.append((name,score,kills))# append name and score to our array
    ...
    def save(self): # action when saving
        with open(self.filename, "w") as f:
            for name, score, kills in self.scores:
                f.write(f"{name},{score},{kills}\n")
                
    def render(self):
        y = 40
        screen.fill(BLACK)
        for i, (name, score, kills) in enumerate(self.scores): # enumerate is used to link the name and score
            text = font.render(f"{i+1}. {name}: {score}", True, WHITE) # 
            text_rect = text.get_rect(center=(WIDTH/2, y))#where we want the rectangle to be 
            print(f"(i+1). {name}: {score}: {kills}")
...
class InputBox:
    ...
    def submit(self):
        highscores.add_score(self.input_entry.get(), player.score, player.kills)
        root.destroy()
...
highscores = Highscores("highscores_mouse.txt")
            ...
            if enemy.distance(missile) < 20:
                ...
                if enemy.health <= 0:
                    ...
                    if player.kills % 10 == 0: #modulus is the remainder
                        ...
                        enemy.type = "boss"
                    elif player.kills % 2 == 0:
                        enemy.surface = pygame.image.load('scary spaceship.png').convert()
                        enemy.surface = pygame.transform.scale(enemy.surface, (60,45)) # change scale
                        enemy.max_health = 600
                        enemy.health = enemy.max_health
                        enemy.type = "scary spaceship"
                        enemy.dy = random.randint(-8,8)
                        enemy.dx = random.randint(30,50) / -10
                    else:
                        enemy.type = "enemy"
                        enemy.dy = 0
